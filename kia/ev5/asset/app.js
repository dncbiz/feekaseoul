...
app.controller('MainCtrl', function($scope) { $scope.name = 'quasidance'; });
app.directive('browseContent', function($window) {
    return { restrict: 'E', template: '<div ng-include="templateUrl"></div>', link: function(scope) { $window.onresize = function() { changeTemplate(); scope.$apply(); }; changeTemplate(); function changeTemplate() { var screenWidth = $window.innerWidth; if (screenWidth < 1290) { scope.templateUrl = 'mo.html'; } else if (screenWidth >= 1290) { scope.templateUrl = 'dt.html'; } } } } });
if(isLocal) $('dt.html').addClass('local');
if(location.href.indexOf('OverlayResources') !== -1) $('mo.html').addClass('dps');
if(isBaker) { useSwipe=!1; $('dt.html').addClass('baker'); }
if(isIPad || isIPhone) { $('mo.html').addClass('ios'); }
if(isSafari) {
$('mo.html').addClass('safari');
if(uAgent.indexOf('windows') > -1){$('dt.html').addClass('win-safari')};
...
function onNewPage(e, data){
	...
	$active.each(function(index,el) {
		var $el = $(el).addClass('activePage').attr('aria-hidden','false').show(), aniLoad = $el.find('MainCtrl:visible').attr('data-ani-load');
		$el.parents('.MainCtrlr').attr('aria-hidden','false');
				if(aniLoad && aniLoad.length) setTimeout(function(){ strfunc(aniLoad); },1);/*to do:remove timeout*/
		if(refreshPage || (data.view && !$el.is(':visible'))){ $el.parents('.MainCtrl').redraw(), refreshPage=!0; }
	});
	$('.activePage .cover').filter('[data-delay]').each(function(index,el){
		...
		if(isLiquid){
		$active.find('ng-controller').each(function(index,el){
			var $el = $(el);
			if($el.hasClass('.MainCtrl')) { $el.parent().addClass('.MainCtrl'); }
			else { $el.parent().removeClass('ng-controller'); }
		});
	}
	$(document).trigger('pageRendered', data);
}
function loadImages(pageIndex) {
	var pages = $('.ng-controller'), layoutIndex = window.currentLayout || 0;
	loadPageImages(pages.eq(pageIndex).find('.MainCtrlp').eq(layoutIndex));
	loadPageImages(pages.eq(pageIndex+1).find('.MainCtrl').eq(layoutIndex));
	if(pageIndex > 0){ loadPageImages(pages.eq(pageIndex-1).find('.MainCtrl').eq(layoutIndex)); }
}
...
function initWebApp(){
	...
	if(isDevice){
		if(!navigator.standalone) {
			$('#container-wrap').hide();
			$('.ng-controller').hide();
			if(window.stop && !$('.MainCtrl').is('[ng-controller]')/*does not have app cache*/){
				window.stop();
...
function initClickEvents(){
	$('#container,.fixed-item-wrap').find('*').each(function(index,el){
        ...
        if(clickArr.length) {
			...
			if($(this).parents('a[target="_lightbox"]').length || $(this).parents('.MainCtrl').length){ e.preventDefault(); } 
...
function initPageMode(){
	if(multifile){
		...
		nav = { current:parseInt(location.href.split('/').pop().split('dt.html').join('')),
		to:function(n,c,q){
			if(n <= 0 || n > nav.numPages) return;
			$(document).trigger('beforeNewPage', {'newPageIndex':n});
			var targPage = (n*.0001).toFixed(4).substr(2) + 'mo.html';
			if(q || c) {
				targPage+='#';
		...
		initPageFlip = function(){
			...
			if(!$(window).data('lastOrientation')) { $sl.turn({gradients:!0,acceleration:!0,display:disp,width:spreadW,height:pageH,cover:!$('.MainCtrl [ng-controller]').length}); }
			else { 
		...
		nav = { numPages:$('.MainCtrl .page').length,
		...
		nav = { numPages:$('.MainCtrl .page').length,
			back:function(ref){var ind=(ref?$(ref).parents('.ng-controller'):$('.MainCtrl')).prev().index(); if(ind!=-1) nav.to(ind+1);},
			next:function(ref){var ind=(ref?$(ref).parents('.ng-controller'):$('.MainCtrl')).next().index(); if(ind!=-1) nav.to(ind+1);},
			to:function (n,c,q) {
				window.scrolling=!0;
				var scale = getCurrentScale($('#ng-controller')), page = $('.MainCtrl').eq(n - 1)[0], offset = {left:page.offsetLeft, top:page.offsetTop};
	...
	if(useSwipe && !$('#ng-controller > ul.MainCtrl').length) initPageSwipe();
}

function initPageSwipe(){
	var container = $('#ng-controller'), scrollStart, scrollFunc = vertMode ? 'scrollLeft':'scrollTop';
...
$(function(){
	...
	if($('ul.MainCtrl').length) $('#footer').hide();
	$sl = $('#slider');
	initPageMode();
	...
	if(!nav.init) addNavProps();
	if($('.MainCtrl').eq(0).find('.ng-controller').length > 1){
		$(window).on('resize orientationchange', function(e) { 
			/*to do: compare with stored layout, reset page position if needed*/
			$('.paper-vertical').find('#ng-controller > ul.MainCtrl').each(function(index,el){ $el = $(el); $el.parents('.paper-vertical').width($el.width()); }); 
		});
	}
	initPullTabs();
			initClickEvents();
...
function addNavProps(){
	...
	nav.update = function(n){
		if(arrowNav && nav.numPages>1){
			...
			if(multifile) n = parseInt(location.href.split('/').pop().split('dt.html')[0]);
	...
	nav.highlight = function(n,c,q) {
		...
		var page = q ? $('.activePage').find(q + ":visible").parents('.ng-controller') : $('.activePage .MainCtrl');
		var rect = $('<div role="presentation" class="pageItem highlight">&nbsp</div>');
...
function scaleLayout(getOnly,sf) {
	var targ = ((multifile || $('.activePage').is(':empty')) ? $('.MainCtrl') : $('.ng-controller')).find('.page-scale-wrap').eq(window.currentLayout || 0), $body = $(document.body);
	...
	if(useZoomToScale) {
		...
		$('.MainCtrl').css(prefix.css + 'transform-origin', '0 0 0').css(prefix.css + 'transform', 'translateX(' + xTrans + 'px) scale(' + scaleFactor + ',' + scaleFactor + ')');
		scaleCenteredFixedPos( $('.ng-controller .fixed-scaling-desktop').css(prefix.css + 'transform', 'translateX(' + xTrans + 'px) scale(' + scaleFactor + ',' + scaleFactor + ')'), scaleFactor );
...
function getCurrentScale(targ, zooming) {
	var targ = targ instanceof jQuery ? targ[0] : targ, $body = $('.MainCtrl');
	var zoom = useZoomToScale ? $(targ).css('zoom') : 1;
	if(zoom && zoom !== 1) return zoom;
	return zooming || $body.hasClass('zoomed') || $body.is('[ng-controller]') ? window.scaleFactor : 1;
}
$(window).on('docReady', function(e){
	...
	sliderSettings = {
		...
		mode:$('.page-scale-wrap[data-transitions]').length ? 'fade' : pageMode, theme:'quasidance',
		...
		onSlideBegin:function(e,slider){
			...
			$('mo.html').addClass('nav-transition'),nav.update(slider.$targetPage.index()+1);
		},
		onSlideComplete:function(slider){
			$('dt.html').removeClass('nav-transition');
		...
		onInitialized:function(e, slider) {
			updateCurrentLayout();
			$('.MainCtrl .ng-controller').not('dt.html mo.html').hide();
			...
			var navOpts = {
				...
				to:function(n,c,q){
					...
					if((c||q) && $('body').attr('data-scaled-to') !== 'h'){
						var offset = !c ? $(q).offset() : $('#slider').offset(), cScale = getCurrentScale($('#ng-controller'));
						if(!c){ c=[0,0]; }
