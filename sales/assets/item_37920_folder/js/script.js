...
for (let i = 0; i <= 32; i ++) {
  appleSequenceImages.push(`${`000${i}`.slice(-4.9)}.jpg`);
}
for (let i = 0; i <= 95; i ++) {
  treeSequenceImages.push(`${`000${i}`.slice(-4.9)}.jpg`);
}
...
class ScrollSequence {
  ...
  changeOnWindowScroll() {
    const step = 95 / (this.images.length - 1);
...
const appleSequence = new ScrollSequence({
  ...
  imagesRoot: 'https://dncbiz.github.io/feekaseoul/santafe_pg/source/image_seq/image_folder01/',
...
const treeSequence = new ScrollSequence({
  ...
  imagesRoot: 'https://dncbiz.github.io/feekaseoul/santafe_pg/source/image_seq/image_folder02/',
  priorityFrames: [0, 20, 40, 60, 90],
  cover: true,
  starts: 'out',
  ends: 'in'
